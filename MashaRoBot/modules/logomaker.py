...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  await event.reply('**Give A Text For The Logo**.')
  return
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./MashaRoBot/resources/logofonts/*")
    ...
    fname = "AloneGod.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"**Made by: @AloneGodRoBot**")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report [Tʜᴀɴɪᴍᴀɪ  Bᴏᴛs](https://t.me/thanimaisupport), {e}')
