...
from MashaRoBot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if not bg_:
        ...
        bg_ = await id_.download_media()
    if not font_:
        fpath_ = glob.glob("MashaRoBot/resources/logofonts/*")
        font_ = random.choice(fpath_)
    if len(name) <= 8:
        fnt_size = 120
        strke = 10
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Alone God](https://t.me/AloneGodRoBot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("MashaRoBot/resources/logofonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if not bg_:
        ...
        bg_ = await id_.download_media()
    if not font_:
        fpath_ = glob.glob("./MashaRoBot/resources/fonts/*")
        font_ = random.choice(fpath_)
    if len(name) <= 8:
        fnt_size = 105
        strke = 8
    ...
    if os.path.exists(bg_):
        os.remove(bg_) 
    if os.path.exists(font_):
        if not font_.startswith("./MashaRoBot/resources/fonts"):
            os.remove(font_)
